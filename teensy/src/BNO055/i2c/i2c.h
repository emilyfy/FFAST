/*
    ------------------------------------------------------------------------------------------------------
    i2c - I2C library for Teensy 3.x & LC
...
#if defined(__AVR__)
#error "Sorry, i2c only works on Teensy LC and 3.x boards.  Use Wire for Teensy 2.0."
#endif
...
public:
    ...
    inline void begin(void)
        { begin_(i2c_struct, bus, I2C_MASTER, 0, 0, 0, 0, I2C_PULLUP_EXT, 100000, I2C_OP_MODE_ISR); }
    //
    ...
    inline void begin(int address)
        { begin_(i2c_struct, bus, I2C_SLAVE, (uint8_t)address, 0, 0, 0, I2C_PULLUP_EXT, 100000, I2C_OP_MODE_ISR); }
    inline void begin(uint8_t address)
        { begin_(i2c_struct, bus, I2C_SLAVE, address, 0, 0, 0, I2C_PULLUP_EXT, 100000, I2C_OP_MODE_ISR); }
    //
    ...
    inline void begin(i2c_mode mode, uint8_t address1, i2c_pins pins=I2C_PINS_DEFAULT,
        ...
        { begin_(i2c_struct, bus, mode, address1, 0, mapSCL(pins), mapSDA(pins), pullup, rate, opMode); }

    inline void begin(i2c_mode mode, uint8_t address1, uint8_t pinSCL, uint8_t pinSDA,
                      i2c_pullup pullup=I2C_PULLUP_EXT, uint32_t rate=400000, i2c_op_mode opMode=I2C_OP_MODE_ISR)
        { begin_(i2c_struct, bus, mode, address1, 0, pinSCL, pinSDA, pullup, rate, opMode); }

    inline void begin(i2c_mode mode, uint8_t address1, uint8_t address2, i2c_pins pins=I2C_PINS_DEFAULT,
                      i2c_pullup pullup=I2C_PULLUP_EXT, uint32_t rate=400000, i2c_op_mode opMode=I2C_OP_MODE_ISR)
        { begin_(i2c_struct, bus, mode, address1, address2, mapSCL(pins), mapSDA(pins), pullup, rate, opMode); }

    inline void begin(i2c_mode mode, uint8_t address1, uint8_t address2, uint8_t pinSCL, uint8_t pinSDA,
                      i2c_pullup pullup=I2C_PULLUP_EXT, uint32_t rate=400000, i2c_op_mode opMode=I2C_OP_MODE_ISR)
        { begin_(i2c_struct, bus, mode, address1, address2, pinSCL, pinSDA, pullup, rate, opMode); }
    ...
    //
    inline uint8_t setOpMode(i2c_op_mode opMode) { return setOpMode_(i2c_struct, bus, opMode); }

    // ------------------------------------------------------------------------------------------------------
    // Set I2C rate (base routine)
    //
    static void setRate_(struct i2cStruct* i2c_struct, uint32_t busFreq, uint32_t i2cFreq);
    //
    ...
    //        2          2        100k
    //
    inline void setRate(uint32_t busFreq, uint32_t i2cFreq) { setRate_(i2c_struct, busFreq, i2cFreq); }
    // return value and i2c_rate enum no longer needed (deprecated form, always returns 1)
    inline uint8_t setRate(uint32_t busFreq, i2c_rate rate) { setRate_(i2c_struct, busFreq, (uint32_t)rate); return 1; }
    // Wire compatibility
    inline void setClock(uint32_t i2cFreq)
    {
        #if defined(__MKL26Z64__) // LC
            if(i2c_struct->currentSCL == 22)
                setRate_(i2c_struct, (uint32_t)F_CPU, i2cFreq); // LC Wire1 bus uses system clock (F_CPU) instead of bus clock (F_BUS)
            else
                setRate_(i2c_struct, (uint32_t)F_BUS, i2cFreq);
        #else
            setRate_(i2c_struct, (uint32_t)F_BUS, i2cFreq);
        #endif
    }
    // i2c version of setClock (deprecated)
    ...
    // parameters: none
    inline uint32_t getClock(void) { return i2c_struct->currentRate; }

    // ------------------------------------------------------------------------------------------------------
    // Configure I2C pins (base routine)
    //
    static uint8_t pinConfigure_(struct i2cStruct* i2c_struct, uint8_t bus, uint8_t pinSCL, uint8_t pinSDA, i2c_pullup pullup, uint8_t reconfig=1);
    //
    // ------------------------------------------------------------------------------------------------------
    ...
    inline uint8_t pinConfigure(i2c_pins pins, i2c_pullup pullup=I2C_PULLUP_EXT)
        { return pinConfigure_(i2c_struct, bus, mapSCL(pins), mapSDA(pins), pullup, 1); }
    inline uint8_t pinConfigure(uint8_t pinSCL, uint8_t pinSDA, i2c_pullup pullup=I2C_PULLUP_EXT)
        { return pinConfigure_(i2c_struct, bus, pinSCL, pinSDA, pullup, 1); }
    //
    // ------------------------------------------------------------------------------------------------------
    ...
    inline void setSCL(uint8_t pin) { pinConfigure_(i2c_struct, bus, pin, i2c_struct->currentSDA, i2c_struct->currentPullup, 1); }
    inline void setSDA(uint8_t pin) { pinConfigure_(i2c_struct, bus, i2c_struct->currentSCL, pin, i2c_struct->currentPullup, 1); }
    //
    // ------------------------------------------------------------------------------------------------------
    // Get SCL/SDA - get the current SCL or SDA pin
    // return: pin used
    // 
    inline uint8_t getSCL(void) { return i2c_struct->currentSCL; }
    inline uint8_t getSDA(void) { return i2c_struct->currentSDA; }
    ...
    inline void setDefaultTimeout(uint32_t timeout) { i2c_struct->defTimeout = timeout; }
    ...
    inline void resetBus(void) { resetBus_(i2c_struct, bus); }
    ...
    //
    inline uint8_t endTransmission(i2c_stop sendStop=I2C_STOP, uint32_t timeout=0) { return endTransmission(i2c_struct, bus, sendStop, timeout); }
    inline uint8_t endTransmission(uint8_t sendStop) { return endTransmission(i2c_struct, bus, (i2c_stop)sendStop, 0); } // Wire compatibility
    ...
    //     ^i2c_stop = I2C_NOSTOP, I2C_STOP (default STOP)
    //
    inline void sendTransmission(i2c_stop sendStop=I2C_STOP) { sendTransmission_(i2c_struct, bus, sendStop, 0); }
    ...
    inline size_t requestFrom(uint8_t addr, size_t len, i2c_stop sendStop=I2C_STOP, uint32_t timeout=0)
        { return requestFrom_(i2c_struct, bus, addr, len, sendStop, timeout); }
    inline size_t requestFrom(int addr, int len)
        { return requestFrom_(i2c_struct, bus, (uint8_t)addr, (size_t)len, I2C_STOP, 0); } // Wire compatibility
    inline uint8_t requestFrom(uint8_t addr, uint8_t len, uint8_t sendStop=1)
        { return (uint8_t)requestFrom_(i2c_struct, bus, addr, (size_t)len, (i2c_stop)sendStop, 0); } // Wire compatibility
    ...
    //     ^i2c_stop = I2C_NOSTOP, I2C_STOP (default STOP)
    //
    inline void sendRequest(uint8_t addr, size_t len, i2c_stop sendStop=I2C_STOP) { sendRequest_(i2c_struct, bus, addr, len, sendStop, 0); }
    ...
    //
    inline i2c_status status(void) { return i2c_struct->currentStatus; }

    // ------------------------------------------------------------------------------------------------------
    // Return Status (base routine)
    //
    static uint8_t done_(struct i2cStruct* i2c_struct);
    //
    ...
    //
    inline uint8_t done(void) { return done_(i2c_struct); }

    // ------------------------------------------------------------------------------------------------------
    // Return Status (base routine)
    //
    static uint8_t finish_(struct i2cStruct* i2c_struct, uint8_t bus, uint32_t timeout);
    //
    ...
    //      ^timeout = timeout in microseconds (default 0 = infinite wait)
    //
    inline uint8_t finish(uint32_t timeout=0) { return finish_(i2c_struct, bus, timeout); }
    ...
    //
    inline int available(void) { return i2c_struct->rxBufferLength - i2c_struct->rxBufferIndex; }

    // ------------------------------------------------------------------------------------------------------
    // Read (base routine)
    //
    static int read_(struct i2cStruct* i2c_struct);
    //
    // Read - returns next data byte (signed int) from Rx buffer
    // return: data, -1 if buffer empty
    //
    inline int read(void) { return read_(i2c_struct); }
    ...
    //
    static size_t read_(struct i2cStruct* i2c_struct, uint8_t* data, size_t count);
    inline size_t read(uint8_t* data, size_t count) { return read_(i2c_struct, data, count); }
    inline size_t read(char* str, size_t count) { return read_(i2c_struct, (uint8_t*)str, count); }
    ...
    // return: data, -1 if buffer empty
    //
    inline int peek(void) { return peek_(i2c_struct); }

    // ------------------------------------------------------------------------------------------------------
    // Read Byte (base routine)
    //
    static uint8_t readByte_(struct i2cStruct* i2c_struct);
    //
    // Read Byte - returns next data byte (uint8_t) from Rx buffer
    // return: data, 0 if buffer empty
    //
    inline uint8_t readByte(void) { return readByte_(i2c_struct); }

    // ------------------------------------------------------------------------------------------------------
    // Peek Byte (base routine)
    //
    static uint8_t peekByte_(struct i2cStruct* i2c_struct);
    //
    // Peek Byte - returns next data byte (uint8_t) from Rx buffer without removing it from Rx buffer
    // return: data, 0 if buffer empty
    //
    inline uint8_t peekByte(void) { return peekByte_(i2c_struct); }
    ...
    //
    inline uint8_t getRxAddr(void) { return i2c_struct->rxAddr; }

    // ------------------------------------------------------------------------------------------------------
    // Set callback function for Master Tx
    //
    inline void onTransmitDone(void (*function)(void)) { i2c_struct->user_onTransmitDone = function; }

    // ------------------------------------------------------------------------------------------------------
    // Set callback function for Master Rx
    //
    inline void onReqFromDone(void (*function)(void)) { i2c_struct->user_onReqFromDone = function; }

    // ------------------------------------------------------------------------------------------------------
    // Set callback function for Slave Rx
    //
    inline void onReceive(void (*function)(size_t len)) { i2c_struct->user_onReceive = function; }

    // ------------------------------------------------------------------------------------------------------
    // Set callback function for Slave Tx
    //
    inline void onRequest(void (*function)(void)) { i2c_struct->user_onRequest = function; }

    // ------------------------------------------------------------------------------------------------------
    // Set callback function for Errors
    //
    inline void onError(void (*function)(void)) { i2c_struct->user_onError = function; }
    ...
    //
    inline uint32_t getErrorCount(i2c_err_count counter) { return i2c_struct->errCounts[counter]; }
    // ------------------------------------------------------------------------------------------------------
    ...
    //                I2C_ERRCNT_ARBL, I2C_ERRCNT_NOT_ACQ, I2C_ERRCNT_DMA_ERR
    //
    inline void zeroErrorCount(i2c_err_count counter) { i2c_struct->errCounts[counter] = 0; }
...
/*
   ...
   ------------------------------------------------------------------------------------------------------
    ...
    - (v1-v4) Modified 26Feb13 by Brian (nox771 at gmail.com)
        ...
        - Separated Tx/Rx buffer sizes for asymmetric devices (adjustable in i2c.h)
