...
//
i2c::i2c(uint8_t i2c_bus)
{
    bus = i2c_bus;
    i2c_struct = &i2cData[bus];
}
i2c::~i2c()
{
    // if DMA active, delete DMA object
    if(i2c_struct->opMode == I2C_OP_MODE_DMA)
        delete i2c_struct->DMA;
}
...
//
void i2c::begin_(struct i2cStruct* i2c, uint8_t bus, i2c_mode mode, uint8_t address1, uint8_t address2,
...
//      offset = array offset
//
uint8_t i2c::validPin_(uint8_t bus, uint8_t pin, uint8_t offset)
{
...
//
uint8_t i2c::setOpMode_(struct i2cStruct* i2c, uint8_t bus, i2c_op_mode opMode)
{
...
//        2          2        100k
//
void i2c::setRate_(struct i2cStruct* i2c, uint32_t busFreq, uint32_t i2cFreq)
{
...
uint8_t i2c::pinConfigure_(struct i2cStruct* i2c, uint8_t bus, uint8_t pinSCL, uint8_t pinSDA, i2c_pullup pullup, uint8_t reconfig)
{
...
//
uint8_t i2c::acquireBus_(struct i2cStruct* i2c, uint8_t bus, uint32_t timeout, uint8_t& forceImm)
{
...
// return: none
//
void i2c::resetBus_(struct i2cStruct* i2c, uint8_t bus)
{
...
{
    i2c_struct->txBuffer[0] = (address << 1); // store target addr
    i2c_struct->txBufferLength = 1;
    clearWriteError(); // clear any previous write error
    i2c_struct->currentStatus = I2C_WAITING; // reset status
}
...
//      timeout = timeout in microseconds
//
uint8_t i2c::endTransmission(struct i2cStruct* i2c_struct, uint8_t bus, i2c_stop sendStop, uint32_t timeout)
{
    sendTransmission_(i2c_struct, bus, sendStop, timeout);

    // wait for completion or timeout
    finish_(i2c_struct, bus, timeout);
...
//
void i2c::sendTransmission_(struct i2cStruct* i2c, uint8_t bus, i2c_stop sendStop, uint32_t timeout)
{
...
//      timeout = timeout in microseconds
//
size_t i2c::requestFrom_(struct i2cStruct* i2c, uint8_t bus, uint8_t addr, size_t len, i2c_stop sendStop, uint32_t timeout)
{
...
//      timeout = timeout in microseconds (only used for Immediate operation)
//
void i2c::sendRequest_(struct i2cStruct* i2c, uint8_t bus, uint8_t addr, size_t len, i2c_stop sendStop, uint32_t timeout)
{
...
//
uint8_t i2c::done_(struct i2cStruct* i2c)
{
...
//      timeout = timeout in microseconds
//
uint8_t i2c::finish_(struct i2cStruct* i2c, uint8_t bus, uint32_t timeout)
{
    elapsedMicros deltaT;
...
{
    if(i2c_struct->txBufferLength < I2C_TX_BUFFER_LENGTH)
    {
        i2c_struct->txBuffer[i2c_struct->txBufferLength++] = data;
        return 1;
    }
    setWriteError();
    return 0;
...
//
size_t i2c::write(const uint8_t* data, size_t count)
{
    if(i2c_struct->txBufferLength < I2C_TX_BUFFER_LENGTH)
    {
        size_t avail = I2C_TX_BUFFER_LENGTH - i2c_struct->txBufferLength;
        uint8_t* dest = i2c_struct->txBuffer + i2c_struct->txBufferLength;
        ...
        i2c_struct->txBufferLength += count;
        return count;
    }
    setWriteError();
    return 0;
...
//
int i2c::read_(struct i2cStruct* i2c_struct)
{
    if(i2c_struct->rxBufferIndex >= i2c_struct->rxBufferLength) return -1;
    return i2c_struct->rxBuffer[i2c_struct->rxBufferIndex++];
}
...
//      count = number of bytes to write
//
size_t i2c::read_(struct i2cStruct* i2c_struct, uint8_t* data, size_t count)
{
    if(i2c_struct->rxBufferLength > i2c_struct->rxBufferIndex)
    {
        size_t avail = i2c_struct->rxBufferLength - i2c_struct->rxBufferIndex;
        uint8_t* src = i2c_struct->rxBuffer + i2c_struct->rxBufferIndex;

        if(count > avail) count = avail; // truncate to data avail if needed
        memcpy(data, src, count);
        i2c_struct->rxBufferIndex += count;
        return count;
    }
    return 0;
}
...
// return: data, -1 if buffer empty
//
int i2c::peek_(struct i2cStruct* i2c_struct)
{
    if(i2c_struct->rxBufferIndex >= i2c_struct->rxBufferLength) return -1;
    return i2c_struct->rxBuffer[i2c_struct->rxBufferIndex];
}
...
//
uint8_t i2c::readByte_(struct i2cStruct* i2c_struct)
{
    if(i2c_struct->rxBufferIndex >= i2c_struct->rxBufferLength) return 0;
    return i2c_struct->rxBuffer[i2c_struct->rxBufferIndex++];
}
...
// return: data, 0 if buffer empty
//
uint8_t i2c::peekByte_(struct i2cStruct* i2c_struct)
{
    if(i2c_struct->rxBufferIndex >= i2c_struct->rxBufferLength) return 0;
    return i2c_struct->rxBuffer[i2c_struct->rxBufferIndex];
}
...
{
    I2C0_INTR_FLAG_ON;
    i2c_isr_handler(&(i2c::i2cData[0]),0);
    I2C0_INTR_FLAG_OFF;
}
#if I2C_BUS_NUM >= 2
    ...
    {
        I2C1_INTR_FLAG_ON;
        i2c_isr_handler(&(i2c::i2cData[1]),1);
        I2C1_INTR_FLAG_OFF;
    }
#endif
#if I2C_BUS_NUM >= 3
    ...
    {
        I2C2_INTR_FLAG_ON;
        i2c_isr_handler(&(i2c::i2cData[2]),2);
        I2C2_INTR_FLAG_OFF;
    }
#endif
#if I2C_BUS_NUM >= 4
    ...
    {
        I2C3_INTR_FLAG_ON;
        i2c_isr_handler(&(i2c::i2cData[3]),3);
        I2C3_INTR_FLAG_OFF;
    }
#endif
        ...
        if(status & I2C_S_IAAS)
        {
            ...
            else
            {
                ...
                // setup SDA-rising ISR - required for STOP detection in Slave Rx mode for 3.0/3.1/3.2
                    ...
                    i2c->irqCount = 0;
                    attachInterrupt(i2c->currentSDA, (bus == 0) ? i2c::sda0_rising_isr : i2c::sda1_rising_isr, RISING);
                #elif defined(__MK20DX128__) || defined(__MK20DX256__) // 3.0/3.1/3.2 (single-bus)
                    i2c->irqCount = 0;
                    attachInterrupt(i2c->currentSDA, i2c::sda0_rising_isr, RISING);
...
// I2C0 SDA ISR
void i2c::sda0_rising_isr(void)
{
    i2c::sda_rising_isr_handler(&(i2c::i2cData[0]),0);
}

#if I2C_BUS_NUM >= 2
    // I2C1 SDA ISR
    void i2c::sda1_rising_isr(void)
    {
        i2c::sda_rising_isr_handler(&(i2c::i2cData[1]),1);
    }
#endif
...
//
void i2c::sda_rising_isr_handler(struct i2cStruct* i2c, uint8_t bus)
{
...
i2c Wire  = i2c(0);       // I2C0
#if I2C_BUS_NUM >= 2
    i2c Wire1 = i2c(1);   // I2C1
#endif
#if I2C_BUS_NUM >= 3
    i2c Wire2 = i2c(2);   // I2C2
#endif
#if I2C_BUS_NUM >= 4
    i2c Wire3 = i2c(3);   // I2C3
#endif
