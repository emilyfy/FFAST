...
namespace steadystate {
    #include "ilqr_params_steadystate.h"
    // extern "C" {
...
void iLQR::_setOpSteadyState(ros::NodeHandle nh, ros::NodeHandle pnh)
{
    // set values in Op_
    ...
    Op_->log_z = NULL;
    Op_->log_cost = NULL;
    if (_assignParamsSteadyState(nh, pnh)) return;
    standard_parameters(Op_);
    Op_->n_hor = HORIZON_STEADYSTATE;
    Op_->max_iter = MAX_ITER;

    // allocate memory for Op_
    for (int i=0; i<NUMBER_OF_THREADS+1; i++) Op_->trajectories[i].t = (trajEl_t *) malloc(sizeof(trajEl_t)*HORIZON_STEADYSTATE);
    Op_->multipliers.t = (multipliersEl_t *) malloc(sizeof(multipliersEl_t)*(HORIZON_STEADYSTATE+1));
    Op_->nominal = (traj_t *) malloc(sizeof(traj_t));
    Op_->nominal->t = (trajEl_t *) malloc(sizeof(trajEl_t)*HORIZON_STEADYSTATE);
    for (int i=0; i<NUMBER_OF_THREADS; i++) {
        Op_->candidates[i] = (traj_t *) malloc(sizeof(traj_t));
        Op_->candidates[i]->t = (trajEl_t *) malloc(sizeof(trajEl_t)*HORIZON_STEADYSTATE);
    }

    // wait for equilibrium to be set and set goal in Op_
    ros::Duration(2).sleep();
    Op_->p[P_GOAL_IDX_STEADYSTATE] = &equilibrium_[2];
}

int iLQR::_assignParamsSteadyState(ros::NodeHandle nh, ros::NodeHandle pnh)
{
...
void iLQR::_ilqrSteadyState(const ilqr_msgs::IlqrInput::ConstPtr& msg)
{
    ...
    for (i=0;i<msg->remainingcommands.size();i++) {
        u0_steadystate_[2*i] = msg->remainingcommands[i].speed;
        u0_steadystate_[2*i+1] = msg->remainingcommands[i].steering_angle;
    }
    for (;i<N_-1;i++) {
        u0_steadystate_[2*i] = equilibrium_[0];
        u0_steadystate_[2*i+1] = equilibrium_[1];
    }
    ...
    if (!init_opt(Op_)) {
        ...
        for (int c=0; c<N_-1; c++)
            for (int r=0; r<N_U; r++)
                Op_->nominal->t[c].u[r] = u0_steadystate_[MAT_IDX(r, c, N_U)];
