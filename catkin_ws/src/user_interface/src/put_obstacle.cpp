...
int main(int argc, char ** argv)
{
    ...
    ros::Publisher pub_obs = nh.advertise<obstacle_detector::Obstacles>("/masked_obstacles", 10);
    ...
    char str[28];
    float pos_x, pos_y, vel_x, vel_y, delay;
    int valid = 0;
    ...
    while (ros::ok() && valid < 4)
    {
        ...
        scanf("%s", str);
        valid = sscanf(str, "%f,%f,%f,%f,%f", &pos_x, &pos_y, &vel_x, &vel_y, &delay);
    }
    ...
    double x = pos_x;
    double y = vel_y > 0 ? -fabs(pos_y) : fabs(pos_y);

    ROS_INFO("putting obstacle at [%f, %f] with velocity [%f, %f].", x, y, vel_x, vel_y);
    obs.circles[0].center.x = state->pose.pose.position.x + x;
    ...
    ros::Rate r(1/dt);
    while (ros::ok() && fabs(obs.circles[0].center.y) <= fabs(pos_y))
    {
