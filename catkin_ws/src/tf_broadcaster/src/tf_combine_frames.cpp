...
void odomCombinedCallback(const geometry_msgs::PoseWithCovarianceStamped& odom_combined)
{
    if (ros::Time::now() - last_map_cb_time > ros::Duration(1)) return;
...
int main(int argc, char** argv){
    ...
    ros::Subscriber map_sub = nh.subscribe("/pose_with_covariance_stamped",10,mapCallback);
